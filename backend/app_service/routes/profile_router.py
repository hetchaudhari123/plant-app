...
async def route_update_name(
    ...
    await update_profile_name(
        user_id=user_id, first_name=payload.first_name, last_name=payload.last_name
    )
...
class DeleteAccountSchema(BaseModel):
    password: str = Field(
        ..., min_length=1, description="User's password for verification"
    )
...
async def route_update_farm_size(
    payload: UpdateFarmSizeSchema, current_user=Depends(require_user)
):
