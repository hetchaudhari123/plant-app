...
async def send_otp(
    ...
    body = template.render(
        email=email, otp=otp_code, expiry_minutes=settings.OTP_EXPIRE_MINUTES
    )
...
async def generate_otp_token(user_id: str, email: EmailStr, new_email: EmailStr):
    ...
    return {"otp_token": token, "expires_at": expires_at}
...
async def resend_email_change_otp(user_id: str):
    ...
    await db_conn.otp_tokens_collection.update_one(
        {"_id": token_doc["_id"]}, {"$set": {"resend_count": resend_count}}
    )
...
async def request_signup_otp(
    email: EmailStr, first_name: str, last_name: str, password: str
):
    """
    ...
    if existing_otp_token:
        ...
        await db_conn.otp_tokens_collection.delete_one(
...
async def signup_user(email: EmailStr, otp_code: str):
    ...
    if not otp_token:
        raise HTTPException(
            status_code=400, detail="Signup session not found or expired"
        )
    ...
    if expires_at < datetime.now(timezone.utc):
        ...
        raise HTTPException(
            status_code=400, detail="OTP has expired. Please restart signup process"
        )
    ...
    profile_pic_url = (
        f"https://api.dicebear.com/5.x/initials/svg?seed={first_name}%20{last_name}"
    )
...
async def resend_signup_otp(email: EmailStr):
    ...
    if otp_token.get("resend_count", 0) >= settings.RESEND_OTP_LIMIT:
        ...
        raise HTTPException(
            status_code=429,
            detail="Resend limit reached. Please restart signup process",
        )
    # 4) Increment resend count
    new_resend_count = otp_token.get("resend_count", 0) + 1
    await db_conn.otp_tokens_collection.update_one(
        {"_id": otp_token["_id"]}, {"$set": {"resend_count": new_resend_count}}
    )
