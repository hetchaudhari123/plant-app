...
export function ConfirmEmailChange() {
  ...
  useEffect(() => {
    ...
    const storedOldEmail = localStorage.getItem('oldEmail');

    if (storedNewEmail) {
      setNewEmail(storedNewEmail);
    } else {
      // If no pending email change found, redirect back to profile
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      await verifyEmailUpdateOtp({ otp_code: otpString, new_email: newEmail, old_email: oldEmail });
    ...
    <div className="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <h2 className="text-3xl text-gray-900">Verify New Email</h2>
          <p className="mt-2 text-gray-600">
            Enter the 6-digit code sent to {newEmail}
          </p>
          {oldEmail && (
            <p className="mt-1 text-sm text-gray-500">
              Changing from: {oldEmail}
            </p>
          )}
        </div>
